const { generateWAMessageFromContent, downloadMediaMessage, downloadContentFromMessage } = require('@whiskeysockets/baileys')
...
/**
 * Download media from message (Updated 2024 with fallback handling)
 */
async function downloadMedia(message, socket) {
    try {
        ...
        // 2024 Fix: Handle known decryption errors with fallback
        if (error.message?.includes('BAD_DECRYPT') || error.message?.includes('OSSL') || error.message?.includes('decrypt')) {
            console.log('🔄 Attempting fallback download method for decryption issue...')
            try {
                // Import here to avoid circular dependency issues
                const { downloadContentFromMessage } = require('@whiskeysockets/baileys')
                const fallbackBuffer = await downloadContentFromMessage(message, 'buffer')
        ...
        console.error('All download methods failed for media')
        return null
    }
}
