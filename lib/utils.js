...
async function writeExif(buffer, metadata) {
  try {
    ...
    const exifAttr = {
      ...
      'Exif': {
        [piexif.ExifIFD.UserComment]: piexif.helper.UserCommentEncode(`sticker-pack-id\x00${packname}\x00sticker-pack-name\x00${packname}\x00sticker-author-name\x00${author}\x00`)
      }
    }
    
    const exifData = piexif.dump(exifAttr)
    
    // Convert WebP to base64 and add EXIF
    const base64Data = 'data:image/webp;base64,' + webpBuffer.toString('base64')
    const newData = piexif.insert(exifData, base64Data)
    
    // Convert back to buffer
    const finalBuffer = Buffer.from(newData.replace(/^data:image\/webp;base64,/, ''), 'base64')
