...
async function writeExif(buffer, metadata) {
  try {
    ...
    // Create user comment string manually for WhatsApp sticker metadata
    const userComment = `sticker-pack-id\x00${packname}\x00sticker-pack-name\x00${packname}\x00sticker-author-name\x00${author}\x00`
    
    const exifAttr = {
      '0th': {
        [piexif.ImageIFD.Make]: packname,
