...
async function writeExif(buffer, metadata) {
  try {
    ...
    // Create metadata object for WebP
    ...
    // Convert metadata to EXIF format that WhatsApp recognizes
    const metadataStr = JSON.stringify(stickerMetadata)
    
    // Use node-webpmux to add metadata to WebP
    const img = new webpmux.Image()
    img.load(buffer)
    
    // Add EXIF chunk with sticker metadata
    img.exif = Buffer.from(metadataStr, 'utf8')
