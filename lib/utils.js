...
async function writeExif(buffer, metadata) {
  try {
    ...
    // Resize image without black background - use transparent background and inside fit
    ...
    // For WhatsApp stickers, we need to use a different approach
    // Create a simple metadata buffer that WhatsApp can read
    ...
    // Create a custom metadata chunk for WebP
    const metadataBuffer = Buffer.from(metadataJson, 'utf-8')
    const metadataChunk = Buffer.concat([
    ...
    metadataChunk.writeUInt32LE(metadataBuffer.length, 4)
    
    // Try to insert metadata into WebP
    let finalBuffer = stickerBuffer
    
    // Alternative: Use WhatsApp's expected metadata format directly in the buffer
    const webpWithMeta = Buffer.concat([
      stickerBuffer.slice(0, 12), // WebP header
      metadataChunk,
      stickerBuffer.slice(12)
    ])
    
    console.log(`✅ Added sticker metadata: Pack="${packname}", Author="${author}"`)
    return stickerBuffer // Return original WebP for now since metadata embedding is complex
    
  } catch (error) {
    console.log('⚠️ Failed to add metadata, sending without:', error.message)
