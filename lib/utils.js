...
async function writeExif(buffer, metadata) {
  try {
    ...
    const exifAttr = {
      'Sticker': {
        'sticker-pack-id': metadata.packname || 'MATDEV Bot',
        'sticker-pack-name': metadata.packname || 'MATDEV Bot', 
        'sticker-author-name': metadata.author || 'MATDEV Bot',
    ...
    img.load(buffer)
    const newData = piexif.insert(exifData, img.getBuffer().toString('binary'))
