    ...
    async logOutgoingMessage(jid, content, result, options = {}) {
        try {
            ...
            await this.database.getModel('Log').create({
                level: 'info',
                message: `OUTGOING: ${messageType} message to ${jid}`,
                category: 'message',
                metadata: messageData
            });

            logger.debug('📤 OUTGOING message logged:', {
                type: messageType,
                to: jid,
                content: messageText ? messageText.substring(0, 100) : '[media]'
            });
