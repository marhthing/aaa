    ...
    setupEventHandlers(saveCreds) {
        ...
        this.sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (statusCode === DisconnectReason.badSession || statusCode === DisconnectReason.restartRequired || statusCode === DisconnectReason.loggedOut) {
        ...
        // Handle connection close directly for more granular control
        this.sock.ev.on('connection.close', async (reason) => {
            console.log('❌ WhatsApp connection lost');
            logger.warn('Connection closed:', reason);

            this.isConnected = false;

            // Only clear session on authentication failures (401) or logout
            // Don't clear on temporary connection errors (like 515 stream errors)
            if (reason?.output?.statusCode === 401 || reason?.tag === 'logout') {
                console.log('🔄 Session expired or logged out - clearing session');
                console.log('📱 Need to reconnect WhatsApp');
                await this.clearSession();
            } else {
                console.log('🔄 Temporary connection error - attempting to reconnect without clearing session');
            }
