...
class Bot extends EventEmitter {
    ...
    setupEventHandlers(saveCreds) {
            ...
            if (connection === 'close') {
                ...
                if (userJid) {
                    try {
                        ...
                        // Update session ID and SUDO_USERS in .env and runtime config
                        // Get the full JID with device ID from sock.user
                        const fullUserJid = this.sock.user?.id || userJid;
                        await this.updateSessionId(fullUserJid);
        ...
        this.sock.ev.on('messages.upsert', async (m) => {
            try {
                console.log('🔔 RAW MESSAGE EVENT RECEIVED:', JSON.stringify(m, null, 2));
    ...
    async sendMessage(jid, content, options = {}) {
        try {
            ...
            await this.logOutgoingMessage(jid, content, result, options);
     ...
     */
    async logOutgoingMessage(jid, content, result, options = {}) {
        try {
            const messageType = this.getContentType(content);
            const messageText = this.extractTextFromContent(content);

            const messageData = {
                messageId: result?.key?.id || 'unknown',
                remoteJid: jid,
                fromMe: true,
                participant: this.sock.user?.id || '',
            ...
            // Save to logs table
            await this.database.getModel('Log').create({
                level: 'info',
                message: `Outgoing: ${messageType}`,
                category: 'message',
                metadata: messageData
            });

        } catch (error) {
            logger.error('Failed to log outgoing message:', error);
        }
    }
    ...
    extractTextFromContent(content) {
        ...
        if (content.video?.caption) return content.video.caption;
        if (content.document?.caption) return content.document.caption;
        if (content.react?.text) return `Reaction: ${content.react.text}`;
        return '';
    }
     ...
     */
    isSudo(userJid) {
        const sudoUsers = this.config.SUDO_USERS || [];
        return sudoUsers.includes(userJid);
    }

    /**
     * Update SESSION_ID in .env file with user's JID
     */
    async updateSessionId(userJid) {
        try {
            const fs = require('fs');
            const path = require('path');
            ...
            if (!fs.existsSync(envPath)) {
                logger.warn('⚠️ .env file not found, cannot update SESSION_ID');
                return;
            }
            ...
            if (envContent.includes('SESSION_ID=')) {
                envContent = envContent.replace(/SESSION_ID=.*/g, `SESSION_ID=${userJid}`);
            } else {
                envContent += `\nSESSION_ID=${userJid}`;
            }

            // Also update SUDO_USERS to include this user if not already included
            if (!this.isSudo(userJid)) {
                if (envContent.includes('SUDO_USERS=')) {
                    const currentSudo = this.config.SUDO_USERS || [];
                    const updatedSudo = [...currentSudo, userJid].join(',');
                    envContent = envContent.replace(/SUDO_USERS=.*/g, `SUDO_USERS=${updatedSudo}`);
                } else {
                    envContent += `\nSUDO_USERS=${userJid}`;
                }
            }
            ...
            this.config.SESSION_ID = userJid;
            if (!this.config.SUDO_USERS.includes(userJid)) {
                this.config.SUDO_USERS.push(userJid);
            }

            logger.info(`🔄 SESSION_ID updated to: ${userJid}`);
            logger.info(`👑 User added to SUDO_USERS: ${userJid}`);
            
            // Also update the config to reflect the new values
            process.env.SESSION_ID = userJid;
            process.env.SUDO_USERS = envContent.match(/SUDO_USERS=(.+)/)?.[1] || userJid;

        } catch (error) {
            logger.error('Failed to update SESSION_ID:', error);
        }
    }
