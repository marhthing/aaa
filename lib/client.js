...
class Client extends EventEmitter {
    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('connection.update', async (update) => {
            ...
            // Handle pairing code - following exact official Baileys documentation pattern
            if ((connection === "connecting" || !!qr) && !this.socket.authState.creds.registered) {
                if (this.authMethod === 'pairing' && this.phoneNumber && !this.pairingCodeRequested) {
                    ...
                    }, 1000) // Small delay to ensure socket readiness
                }
            }
    ...
    async selectAuthenticationMethod() {
        ...
        return new Promise((resolve) => {
            ...
            console.log('1. QR Code (default) - Scan with your phone [RECOMMENDED]')
            console.log('2. 8-Digit Pairing Code - Use phone number [KNOWN ISSUES]')
            console.log('Note: Pairing codes are unreliable in 2024. QR code is recommended.')
            console.log('='.repeat(50))

            rl.question('Enter your choice (1 or 2, press Enter for default): ', async (choice) => {
                if (choice === '2') {
                    // Pairing code option (now option 2)
                    ...
                    // Default to QR code (choice 1 or just Enter)
                    this.authMethod = 'qr'
