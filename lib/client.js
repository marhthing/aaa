        ...
        try {
            ...
            if (text?.startsWith(config.PREFIX)) {
                ...
                if (message.key.fromMe) {
                    ...
                    // CRITICAL FIX: Auto-detect owner LID by checking if sender matches owner in DMs
                    if (!isOwnerMessage && actualSender.endsWith('@lid') && isGroup) {
                        // Check if we've seen this user as owner in direct messages
                        // Look for any recent direct message from the owner to establish LID mapping
                        const ownerBaseNumber = this.ownerJid.split('@')[0].split(':')[0]
                        
                        // Try to find if this LID could belong to the owner by checking message patterns
                        // This is a heuristic approach: if someone uses owner-only commands from an LID,
                        // they're likely the owner using a group LID
                        if (command === 'jid' || command === 'ping' || command === 'status' || command === 'help') {
                            // These are common owner commands - if used from LID, likely the owner
                            logger.info(`🔍 Potential owner LID detected: ${actualSender} using owner command: ${command}`)
                            
                            // Register this LID as owner and grant access
                            this.registerOwnerGroupJid(actualSender)
                            isOwnerMessage = true
                            logger.info(`🔐 Auto-registered potential owner LID: ${actualSender}`)
                        }
                    }
