    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('messages.update', async (updates) => {
            for (const update of updates) {
                ...
                if (update.update.messageStubType === 68 || // REVOKE message type
                    (update.update.message && update.update.message.protocolMessage?.type === 0)) { // REVOKE protocol
                    ...
                    try {
                        ...
                        if (config.DEBUG_MODE) {
                            logger.info('Anti-delete handler error:', error.message)
                        }
                    }
                }
            }
        ...
        try {
            ...
            try {
                ...
                if (antiDeleteModule && antiDeleteModule.trackMessage) {
                    antiDeleteModule.trackMessage(message, text)
                }
            } catch (error) {
