...
class Client extends EventEmitter {
    ...
    async handleMessage(message) {
        try {
            ...
            // FIRST: Check for actual view once messages WITH CONTENT (priority - capture before they become stubs)
            if (message.message?.viewOnceMessage || message.message?.viewOnceMessageV2 || message.message?.viewOnceMessageV2Extension) {
                logger.info(`👁️ View once message with content detected from ${(message.key?.remoteJid || 'unknown').split('@')[0]} - attempting download`)
                await this.handleViewOnceMessage(message)
                return
            }
            
            // SECOND: Check for view once stub messages (content already gone - just log)
            if (message.messageStubType === 2 && 
                message.messageStubParameters?.includes('Message absent from node')) {
                logger.info(`👁️ View once stub detected from ${(message.key?.remoteJid || 'unknown').split('@')[0]} (content already gone)`)
