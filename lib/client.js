...
class Client extends EventEmitter {
    ...
    async connect() {
        try {
            ...
            if (sessionExists) {
                ...
                // Ask for authentication method when no session exists
    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('connection.update', async (update) => {
            ...
            if (qr) {
                // Only show QR if we're using QR method or no method specified
                if (!this.authMethod || this.authMethod === 'qr') {
            ...
            // Handle pairing code for registered devices
            if (connection === "connecting" && !this.socket.authState.creds.registered) {
                if (this.authMethod === 'pairing' && this.phoneNumber) {
                    try {
    ...
    async selectAuthenticationMethod() {
        ...
        if (config.AUTH_METHOD === '1') {
            ...
            return
        } else if (config.AUTH_METHOD === '2' && config.PHONE_NUMBER) {
        ...
        return new Promise((resolve) => {
            ...
            console.log('1. QR Code (default) - Scan with your phone')
            console.log('2. 8-Digit Pairing Code - Use phone number')
            console.log('='.repeat(50))
