...
class Client extends EventEmitter {
    ...
    async handleMessage(message) {
        try {
            ...
            // Only track owner identity, don't register random group JIDs
            const isGroup = messageJid.endsWith('@g.us')
            if (isGroup && message.key.participant && message.key.fromMe) {
                // Only register when fromMe=true (confirmed owner message)
                logger.info(`🔐 Owner message confirmed via fromMe=true: ${message.key.participant}`)
            ...
            if (text?.startsWith(config.PREFIX)) {
                ...
                if (message.key.fromMe) {
                    isOwnerMessage = true
                    // Only register LID if confirmed fromMe
                ...
                // Method 3: Check if sender is in ownerGroupJids (explicitly registered LIDs)
                if (!isOwnerMessage && isGroup && message.key.participant) {
                    isOwnerMessage = this.ownerGroupJids.has(message.key.participant)
                }

                // Method 4: Specific LID registration for known owner LIDs
                // Based on your logs, register your specific LID as owner
                if (!isOwnerMessage && message.key.participant === '250349969522826@lid') {
                    logger.info(`🔐 Registering known owner LID: ${message.key.participant}`)
                    isOwnerMessage = true
                    this.registerOwnerGroupJid(message.key.participant)
                }
    ...
    isOwnerJid(jid) {
        ...
        // Primary check: Compare base phone numbers
        if (baseJidNumber && baseOwnerNumber && baseJidNumber === baseOwnerNumber) {
            return true
        }

        // Secondary check: Direct JID match
        if (jid === this.ownerJid) return true

        // Tertiary check: Socket user match
        if (this.socket?.user?.id && jid === this.socket.user.id) {
            return true
        }

        // Final check: Registered owner group JIDs (only if confirmed by fromMe)
