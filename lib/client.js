        ...
        try {
            ...
            if (!message.message) {
                ...
                if (!jid.includes('@lid') && !jid.includes('@broadcast')) {
                    logger.info(`⚠️ Skipping message without content from: ${jid.split('@')[0]}`)
                }
                return
            }

            // DEBUG: Let's see what message types we're getting
            const messageTypes = Object.keys(message.message || {})
            const jid = message.key?.remoteJid || 'unknown'
            
            // Check for view once messages FIRST (before content filtering)
            const hasViewOnce = message.message.viewOnceMessage || message.message.viewOnceMessageV2 || message.message.viewOnceMessageV2Extension
            
            if (hasViewOnce) {
                logger.info(`🔍 DEBUG - View once detected from ${jid.split('@')[0]}: ${messageTypes.join(', ')}`)
                await this.handleViewOnceMessage(message)
                return // Don't process further since we handled the view once
            }
            
            if (config.DEBUG_MODE && !jid.includes('@lid')) {
                logger.info(`🔍 DEBUG - Message types from ${jid.split('@')[0]}: ${messageTypes.join(', ')}`)
            }
