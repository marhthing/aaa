...
class Client extends EventEmitter {
    ...
    async handleMessage(message) {
        try {
            ...
            // View once stub messages (content already gone - just log event)
            if (message.messageStubType === 2 && 
                message.messageStubParameters?.includes('Message absent from node')) {
                logger.info(`👁️ View once stub detected from ${(message.key?.remoteJid || 'unknown').split('@')[0]} (content already processed)`)
