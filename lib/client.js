...
class Client extends EventEmitter {
            ...
            if ((connection === "connecting" || qr) && !this.socket.authState.creds.registered) {
                if (!this.authMethod || this.authMethod === 'pairing') {
                    if (this.phoneNumber && !this.pairingCodeRequested) {
                        ...
                        // Add delay to ensure socket is ready and prevent rapid regeneration
                        setTimeout(async () => {
                            try {
                                ...
                                // Prevent rapid regeneration by adding cooldown
                                setTimeout(() => {
                                    this.pairingCodeRequested = false
                        ...
                        }, 5000) // Wait 5 seconds before requesting pairing code
                    }
                }
            }
    ...
    hasAccess(jid, command) {
        ...
        if (jid === this.ownerJid || normalizedJid === normalizedOwnerJid) {
            logger.info(`✅ Access granted: Owner (${normalizedJid} === ${normalizedOwnerJid})`)
            return true
        }
