...
class Client extends EventEmitter {
    ...
    async handleMessage(message) {
        try {
            ...
            if (isGroup && message.key.participant) {
                // Check if this is from the owner (either fromMe or identified as owner)
                const isFromOwner = message.key.fromMe || this.isOwnerJid(senderJid)
                if (isFromOwner) {
                    // Register this participant JID as belonging to the owner
            ...
            if (text?.startsWith(config.PREFIX)) {
                ...
                logger.info(`   participant: ${message.key.participant}`)
                ...
                if (message.key.fromMe) {
                    ...
                    // If this is a group message with fromMe=true, register the participant LID
                    if (isGroup && message.key.participant && message.key.participant.endsWith('@lid')) {
                        this.registerOwnerGroupJid(message.key.participant)
                        logger.info(`🔐 Auto-registered owner LID from fromMe message: ${message.key.participant}`)
                    }
                    ...
                    logger.info(`🔍 Checking sender: ${actualSender}`)
                    logger.info(`🔍 isOwnerMessage: ${isOwnerMessage}`)

                    // If this is an owner message in a group, register the participant JID
                    if (isOwnerMessage && isGroup && message.key.participant) {
                        this.registerOwnerGroupJid(message.key.participant)
                    }

                    // Special handling for LID - if this LID is already registered, treat as owner
                    if (!isOwnerMessage && actualSender.endsWith('@lid') && this.ownerGroupJids.has(actualSender)) {
                        logger.info(`✅ Access granted: Registered LID owner ${actualSender}`)
                        await this.handleCommand(message, text)
                        ...
                        logger.info(`🚫 Access denied for ${actualSender}`)
                        
                        // For debugging: if this is a LID and it's not working, let's manually register it
                        // This is a temporary debug solution
                        if (actualSender.endsWith('@lid') && actualSender === '250349969522826@lid') {
                            logger.info(`🔧 DEBUG: Manually registering your LID for testing`)
                            this.registerOwnerGroupJid(actualSender)
                            logger.info(`🔧 DEBUG: Retrying command after registration`)
                            await this.handleCommand(message, text)
                        }
                    }
                }
    ...
    isOwnerJid(jid) {
        ...
        // Check if this is the socket user's JID (when bot owner sends messages)
        ...
        const extractPhoneNumber = (jid) => {
            if (!jid) return ''
            // Split by @ first to get the user part
            const userPart = jid.split('@')[0]
            // Then split by : to remove device suffix or lid suffix
        ...
        // For LID (Link ID) format ending with @lid, we need to check if this LID belongs to the owner
        // The issue is that LID is a privacy feature that hides the actual phone number
        // We cannot reliably map LID back to phone numbers without additional context
        if (jid.endsWith('@lid')) {
            // LIDs are unique identifiers but don't contain phone numbers
            // We need to rely on WhatsApp's own identification methods
            logger.info(`🔍 LID detected: ${jid} - checking against socket user`)
            
            // Check if we can get the real JID from socket.user
            if (this.socket?.user?.id) {
                const socketUserId = this.socket.user.id
                logger.info(`🔍 Socket user ID: ${socketUserId}`)
                
                // If the message was sent fromMe, the LID participant should match
                // This will be handled in the message processing logic
            }
            
            return false // We cannot determine ownership from LID alone
        }
