...
class Client extends EventEmitter {
    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('messages.upsert', async ({ messages, type }) => {
            logger.info(`📨 Received messages.upsert - type: ${type}, count: ${messages.length}`)
            if (type !== 'notify') {
                logger.info(`⚠️ Ignoring non-notify message type: ${type}`)
                return
            }

            for (const message of messages) {
                logger.info(`📨 Processing message: ${message.key?.id} from ${message.key?.remoteJid}`)
    ...
    async handleMessage(message) {
        try {
            ...
            if (!message.message) {
                logger.info('⚠️ Skipping message without content')
                return
            }
            ...
            logger.info(`📨 Message from ${senderJid} in ${jid}: "${text || '[Media/System]'}"`)
            if (config.DEBUG_MODE) {
                logger.info(`📨 Full message details:`, JSON.stringify(message, null, 2))
            }
            ...
            if (config.ENABLE_MESSAGE_ARCHIVE) {
                logger.info(`📁 Archiving message from ${senderJid}`)
                await this.archiveMessage(message, text)
            } else {
                logger.info('📁 Message archival disabled in config')
            }
    ...
    async archiveMessage(message, text) {
        try {
            ...
            logger.info(`📁 Message archived to: ${archivePath} (${messages.length} total messages)`)
