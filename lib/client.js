...
class Client extends EventEmitter {
    ...
    async handleMessage(message) {
        try {
            ...
            // Skip messages from LinkedIn integration (@lid) and other non-WhatsApp sources
            const jid = message.key.remoteJid
            if (jid && jid.includes('@lid')) {
                // Silently skip LinkedIn integration messages
                return
            }

            if (!hasTextContent) {
                // Only log if debug mode is enabled to reduce console noise
            ...
            // Check for view once messages and save them
