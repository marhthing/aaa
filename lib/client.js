    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('messages.update', async (updates) => {
            for (const update of updates) {
                ...
                // WhatsApp sends different types of delete notifications
                const isDeleted = update.update?.messageStubType === 68 || // REVOKE message type
                                 update.update?.message?.protocolMessage?.type === 0 || // REVOKE protocol
                                 update.update?.message?.protocolMessage?.key || // Protocol message with key
                                 (update.update && update.key && !update.update.message) // Message removed
