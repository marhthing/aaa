            ...
            if (isGroup && message.key.participant) {
                ...
                if (isFromOwner) {
                    ...
                    if (participantNumber && ownerNumber && participantNumber === ownerNumber) {
                        ...
                        // Only register LIDs for messages that come with fromMe=true
                        // This ensures only the owner's LIDs get registered automatically
                        if (message.key.fromMe && text && !text.startsWith(config.PREFIX)) {
                            logger.info(`🔐 Auto-registering owner LID from fromMe message: ${message.key.participant}`)
                            this.registerOwnerGroupJid(message.key.participant)
                        }
                    }
                }
