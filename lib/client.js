    ...
    async handleMessage(message) {
        try {
            ...
            if (isGroup && message.key.participant) {
                ...
                if (isFromOwner) {
                ...
                } else if (message.key.participant.endsWith('@lid')) {
                    // For LID messages, check if the base phone number matches the owner
                    ...
                    if (participantNumber && ownerNumber && participantNumber === ownerNumber) {
                        logger.info(`🔐 Auto-registering LID based on phone number match: ${message.key.participant}`)
                        this.registerOwnerGroupJid(message.key.participant)
                        
                        // Send confirmation message
                        setTimeout(async () => {
                            try {
                                await utils.sendMessage(this.socket, messageJid, `✅ Your LID has been automatically registered. You can now use commands!`, { quoted: message })
                            } catch (error) {
                                logger.error('Failed to send LID registration confirmation:', error)
                            }
                        }, 1000)
                    } else {
                        // Try alternative matching - check if LID belongs to owner based on different criteria
                        logger.info(`🔍 Trying alternative LID matching for: ${message.key.participant}`)
                        
                        // For now, let's be more permissive and register any LID when the owner sends a non-command message
                        // This is a temporary solution while we debug the exact matching logic
                        if (text && !text.startsWith(config.PREFIX)) {
                            logger.info(`🔐 Auto-registering LID (permissive mode): ${message.key.participant}`)
                            this.registerOwnerGroupJid(message.key.participant)
                            
                            setTimeout(async () => {
                                try {
                                    await utils.sendMessage(this.socket, messageJid, `✅ Your LID has been registered (permissive mode). You can now use commands!`, { quoted: message })
                                } catch (error) {
                                    logger.error('Failed to send LID registration confirmation:', error)
                                }
                            }, 1000)
            ...
            if (text?.startsWith(config.PREFIX)) {
                ...
                if (message.key.fromMe) {
                    ...
                    let isOwnerMessage = this.isOwnerJid(actualSender)

                    // Security: No heuristic detection to prevent unauthorized access
                    // Only rely on fromMe=true detection which is secure
