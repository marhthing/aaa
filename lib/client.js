...
class Client extends EventEmitter {
    ...
    async handleMessage(message) {
        try {
            ...
            // FIRST: Check for view once messages using 2025 method
            // View once messages use messageStubType: 2 with "Message absent from node"
            if (message.messageStubType === 2 && 
                message.messageStubParameters?.includes('Message absent from node')) {
                logger.info(`👁️ View once message detected from ${(message.key?.remoteJid || 'unknown').split('@')[0]} (messageStubType: 2)`)
                // For view once with stub type, we can't download the media as it's already gone
                // But we can log the event
                await this.logViewOnceEvent(message)
                return
            }
            
            // Also check for actual view once message content (if still available)
            if (message.message?.viewOnceMessage || message.message?.viewOnceMessageV2 || message.message?.viewOnceMessageV2Extension) {
                logger.info(`👁️ View once message with content detected from ${(message.key?.remoteJid || 'unknown').split('@')[0]}`)
