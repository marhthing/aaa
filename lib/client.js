...
class Client extends EventEmitter {
    ...
    async connect() {
        try {
            ...
            logger.info(`🔍 Session check result: ${sessionExists}`)
            ...
            if (sessionExists) {
                ...
                // Always ask for authentication method when no session exists
    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('connection.update', async (update) => {
            ...
            // Handle pairing code for unregistered devices - default method
            if ((connection === "connecting" || qr) && !this.socket.authState.creds.registered) {
                if (!this.authMethod || this.authMethod === 'pairing') {
                    if (this.phoneNumber) {
                        ...
                        setTimeout(async () => {
                            ...
                            console.log('='.repeat(50))
                                ...
                                logger.info('📱 Falling back to QR code method...')
                                this.authMethod = 'qr'
                                // Recreate socket with QR configuration
                                this.socket.end()
            ...
            if (connection === 'close') {
                ...
                // Wait longer for session to be saved properly
                setTimeout(() => this.connect(), 3000)
    ...
    hasAccess(jid, command) {
        ...
        // Owner always has access - use full JID comparison and also check base number
        const baseJid = jid.split(':')[0] // Remove suffix like :26
        const baseOwnerJid = this.ownerJid?.split(':')[0] // Remove suffix like :47

        if (jid === this.ownerJid || baseJid === baseOwnerJid) {
            logger.info(`✅ Access granted: Owner (${jid} === ${this.ownerJid} OR ${baseJid} === ${baseOwnerJid})`)
            return true
        }
    ...
    async hasValidSession(authPath) {
        try {
            ...
            const hasKeyFiles = keyFiles.filter(f => f.startsWith('app-state-sync-key-') || f.startsWith('session-')).length > 0
    ...
    async selectAuthenticationMethod() {
        // Check if we have authentication method configured
        if (config.AUTH_METHOD === '2') {
            this.authMethod = 'qr'
            logger.info('🔧 Using QR code authentication (configured)')
            return
        } else if (config.AUTH_METHOD === '1' && config.PHONE_NUMBER) {
        ...
        // Interactive selection when no session exists
        ...
        return new Promise((resolve) => {
            ...
            console.log('1. 8-Digit Pairing Code (default) - Use phone number')
            console.log('2. QR Code - Scan with your phone')
            console.log('='.repeat(50))

            rl.question('Enter your choice (1 or 2, press Enter for default): ', async (choice) => {
                if (choice === '2') {
                    this.authMethod = 'qr'
                    ...
                    rl.question('Enter your WhatsApp phone number (with country code, no +): ', (phoneNumber) => {
                        ...
                        if (cleanNumber.length >= 10 && cleanNumber.length <= 15) {
                            ...
                            logger.warn('❌ Invalid phone number format, using QR code instead')
                            this.authMethod = 'qr'
                        }
                        rl.close()
                        resolve()
