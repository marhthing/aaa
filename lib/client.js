...
class Client extends EventEmitter {
    ...
    isOwnerJid(jid) {
        ...
        // Handle group participant JIDs (which can end with :lid@domain)
        ...
        // Special case for group participant JIDs that end with "lid"
        // Group participant JIDs format: phoneNumber:lid@g.us or phoneNumber@s.whatsapp.net:lid@g.us
        if (jid.includes('lid@')) {
            const participantPhone = jid.split(':')[0].split('@')[0]
            const ownerPhone = this.ownerJid.split('@')[0].split(':')[0]
