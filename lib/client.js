...
class Client extends EventEmitter {
    ...
    async connect() {
        try {
            ...
            // Create WhatsApp socket with optimized settings for pairing code
            this.socket = makeWASocket({
                auth: state,
                logger: pino({ level: 'silent' }),
                printQRInTerminal: false,
                browser: this.authMethod === 'pairing' ? Browsers.macOS('Chrome') : ['MATDEV Bot', 'Desktop', '1.0.0'],
                defaultQueryTimeoutMs: 60000, // Increased timeout for pairing
    ...
    setupEventHandlers(saveCreds) {
        ...
        this.socket.ev.on('connection.update', async (update) => {
            ...
            // Handle pairing code - improved implementation
            if (connection === "connecting" && this.authMethod === 'pairing' && this.phoneNumber && !this.pairingCodeRequested && !this.socket.authState.creds.registered) {
                this.pairingCodeRequested = true
                
                // Generate pairing code immediately when connecting
                try {
    ...
    async selectAuthenticationMethod() {
        // Always use pairing code when phone number is provided
                ...
                if (choice === '2') {
                    ...
                    rl.question('Enter your WhatsApp phone number (with country code, no +): ', (phoneNumber) => {
                        ...
                        if (cleanNumber.length >= 10 && cleanNumber.length <= 15) {
                            ...
                            logger.warn('❌ Invalid phone number format, please try again')
                            // Try again with better validation
                            this.authMethod = 'pairing'
                            this.phoneNumber = cleanNumber
                        }
                        rl.close()
