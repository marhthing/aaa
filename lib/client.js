...
class Client extends EventEmitter {
        ...
        try {
            ...
            this.pluginWatcher
                ...
                .on('change', async (filePath) => {
                    if (filePath.endsWith('.js')) {
                        logger.info(`🔥 Plugin modified: ${path.basename(filePath)}`)
                        await this.loadPlugins()
                        logger.info(`🎯 Hot reload complete - ${plugins.length} plugins loaded`)
                    }
                })
                .on('unlink', async (filePath) => {
                    if (filePath.endsWith('.js')) {
                        logger.info(`🗑️ Plugin removed: ${path.basename(filePath)}`)
                        await this.loadPlugins()
                        logger.info(`🎯 Hot reload complete - ${plugins.length} plugins loaded`)
                    }
                })
            ...
            logger.warn('Hot reload not available - chokidar not installed:', error.message)
        }
    }
    ...
    shouldReact() {
        try {
            ...
            // Module not loaded or error - default to enabled
        }
